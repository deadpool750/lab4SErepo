...
class NoteTests(APITestCase):
    def setUp(self):
        self.medication = Medication.objects.create(
            name="Test Med", dosage_mg=10, prescribed_per_day=1
        )
    ...
    def test_delete_note(self):
        ...
        create_resp = self.client.post(
            self.list_url, {"medication": self.medication.id, "text": "To be deleted"}
        )
    ...
    def test_update_note_not_allowed(self):
        ...
        # Create a note first
        create_resp = self.client.post(
            self.list_url, {"medication": self.medication.id, "text": "Original Text"}
        )
        note_id = create_resp.data.get("id")
